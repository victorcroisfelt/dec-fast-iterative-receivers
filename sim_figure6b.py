...
for s in range(nsetups):
    ...
    for it, niter in enumerate(niter_range):
        ...
        xhat_soft_sdk = standard_distributed_kaczmarz_receiver(H, y_, SNR, niter=niter)
        xhat_soft_sdk_previous = standard_distributed_kaczmarz_receiver(H, y_, SNR, mu='previous', niter=niter)
        xhat_soft_sdk_proposed = standard_distributed_kaczmarz_receiver(H, y_, SNR, mu='proposed', niter=niter)
        breakpoint()
...
np.savez('mmimo_ber_vs_cycles.npz',
    M=M,
    K=K,
